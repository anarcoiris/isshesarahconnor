...
Mejoras incluidas:
 - AMP (GradScaler) correctamente aplicado
...
def create_sampler_if_needed(dataset: datasets.ImageFolder):
    ...
    class_sample_count = np.array([targets.count(t) for t in sorted(set(targets))])
    # si ratio max/min > 1.5 consideramos desbalance
...
def train_one_epoch(model: nn.Module, dataloader: DataLoader, criterion, optimizer, device,
                    scaler: GradScaler, max_grad_norm: Optional[float] = None) -> Tuple[float, int]:
    model.train()
    running_loss = 0.0
    n_samples = 0
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if tb_writer:
            ...
            # per-class metrics (flatten)
            for k,v in eval_res["report"].items():
                if k.isdigit() or k in train_dataset.class_to_idx:
                    # skip numeric keys handling; better to log macro avg
                    pass
            # log lr
            try:
