#!/usr/bin/env python3
"""
Entrenamiento con transfer learning y AMP + mejoras robustas.

Uso ejemplo:
  python scripts/train_03.py --data data/final --epochs 15 --batch 32 --lr 1e-4 --use_sampler --freeze_epochs 3 --amp

Mejoras incluidas:
 - AMP (GradScaler) con fallback por compatibilidad
 - Freeze/unfreeze progresivo
 - Weighted sampler opcional para clases desbalanceadas
 ...
 - Manejo más robusto de autocast para versiones antiguas
"""
...
def create_sampler_if_needed(dataset: datasets.ImageFolder):
    ...
    if class_sample_count.max() / (class_sample_count.min() + 1e-12) > 1.5:
        # weights per sample = 1 / count[class_of_sample]
        weights = 1.0 / torch.tensor([class_sample_count[t] for t in targets], dtype=torch.double)
        sampler = WeightedRandomSampler(weights, num_samples=len(weights), replacement=True)
        return sampler
    return None

# Dummy GradScaler to use when AMP disabled or unavailable
...
def create_grad_scaler(use_amp: bool, device: torch.device):
    """
    Crear un GradScaler robusto que funcione en distintas versiones de PyTorch.
    Devuelve un scaler real o DummyGradScaler si AMP no está disponible o deshabilitado.
    """
    if not use_amp:
        return DummyGradScaler()
    # intentar crear GradScaler con firma moderna (enabled=...)
    try:
        # preferir no pasar 'device' para evitar TypeError en versiones antiguas
        scaler = GradScaler(enabled=True)
        return scaler
    except TypeError:
        try:
            scaler = GradScaler()  # fallback sin args
            return scaler
        except Exception:
            return DummyGradScaler()

# autocast context wrapper (algunas versiones aceptan enabled=, otras no)
def autocast_ctx(enabled: bool):
    try:
        return autocast(enabled=enabled)
    except TypeError:
        # old signature (no enabled param) -> if disabled we must use nullcontext to avoid autocast
        if enabled:
            return autocast()
        else:
...
def train_one_epoch(model: nn.Module, dataloader: DataLoader, criterion, optimizer, device,
                    scaler, max_grad_norm: Optional[float] = None) -> Tuple[float, int]:
    model.train()
    running_loss = 0.0
    n_samples = 0
    for inputs, labels in dataloader:
        ...
        optimizer.zero_grad()
        with autocast_ctx(isinstance(scaler, GradScaler) or not isinstance(scaler, DummyGradScaler)):
            outputs = model(inputs)
            loss = criterion(outputs, labels)
        # scale may be dummy (returns loss) or GradScaler object
        scaled = scaler.scale(loss)
        scaled.backward()
        if max_grad_norm is not None:
            # unscale if scaler has that method
...
def evaluate(model: nn.Module, dataloader: DataLoader, criterion, device) -> Dict:
    ...
    with torch.no_grad():
        for inputs, labels in dataloader:
            inputs = inputs.to(device, non_blocking=True)
            labels = labels.to(device, non_blocking=True)
            # inference without AMP wrapper (safe)
            outputs = model(inputs)
            loss = criterion(outputs, labels)
...
def main():
    ...
    parser.add_argument('--pretrained', action='store_true', help='Usar pesos preentrenados (pasa --pretrained para activar)')
    ...
    parser.add_argument('--amp', action='store_true', help='Activar AMP (automatic mixed precision)')
    ...
    # distribution by class idx
    from collections import Counter
    idx_counts = Counter([s[1] for s in train_dataset.samples])
    print("Distribución por índice de clase (train):", dict(idx_counts))

    # fix num_classes if mismatch
    ...
    # if no sampler and classes desbalanced, compute class weights for loss
    targets = [s[1] for s in train_dataset.samples]
    class_counts = np.array([targets.count(i) for i in range(len(train_dataset.classes))])
    class_weights = None
    if sampler is None and len(class_counts) > 0 and (class_counts.max() / (class_counts.min() + 1e-12) > 1.5):
        class_weights = torch.tensor((1.0 / (class_counts + 1e-12)) * (class_counts.sum() / class_counts.sum()), dtype=torch.float)
        print("Clases desbalanceadas: aplicando class_weights en la pérdida")
    else:
        class_weights = None
    ...
    # scheduler (try to construct ReduceLROnPlateau compatibly)
    if args.scheduler == 'reduce_on_plateau':
        try:
            scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=3, verbose=True)
        except TypeError:
            # older versions: no verbose arg
            scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=3)
    else:
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=10)

    # create scaler (robusto)
    use_amp = args.amp and (device.type == 'cuda')
    scaler = create_grad_scaler(use_amp, device)
    if isinstance(scaler, DummyGradScaler):
        print("AMP deshabilitado o no disponible -> usando DummyGradScaler (sin mixed precision).")
    else:
        print("AMP activado -> usando GradScaler")
    ...
    if args.resume:
        ...
        # flexible load: accept either dict with key 'model_state' or raw state_dict
        state = ckpt.get('model_state', ckpt)
        try:
            model.load_state_dict(state)
            print(f"Modelo cargado desde {args.resume}")
        except Exception as e:
            print("Warning: no se pudo cargar exactamente el state_dict (compatibilidad):", e)
        try:
            ...
            print("Checkpoint cargado pero no se pudo restaurar optimizer/epoch (posible incompatibilidad)")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        train_loss, n_tr = train_one_epoch(model, train_loader, criterion, optimizer, device, scaler, max_grad_norm=args.grad_clip)
        ...
        if args.scheduler == 'reduce_on_plateau':
            try:
                ...
                # some ReduceLROnPlateau variants expect metric differently; try safe call
                try:
                    scheduler.step(val_loss)
                except Exception:
                    pass
        ...
        try:
            ...
            print("Warning: no se pudo guardar checkpoint:", e)
        ...
        if val_loss < best_val:
            ...
            try:
                ...
                print("Warning: no se pudo guardar best_model:", e)
            epochs_no_improve = 0
        else:
            epochs_no_improve += 1
        ...
        try:
            ...
            print("Warning: no se pudo guardar metrics json:", e)
    ...
    try:
        ...
        print("Warning: no se pudo guardar training_history.csv:", e)
